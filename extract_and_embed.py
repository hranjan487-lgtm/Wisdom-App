...
class EmbeddingFailureError(Exception):
    """Raised when embedding failures are too widespread to silently continue."""


def embed_chunks(chunks, client, max_retries=5):
    """Embed all chunks in batches, adding an 'embedding' field to each.
    Retries with backoff long enough to respect per-minute rate limits
    (not just a few seconds) - a 429 needs to wait out the actual window,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    print(f"  ERROR: embedding failed for batch at {batch_start} after {max_retries} attempts: {e}")
                    for c in batch:
                        c["embedding"] = None
                else:
                    wait = 65 if is_rate_limit else (2 ** attempt)
                    print(f"  Retrying batch at {batch_start} in {wait}s (attempt {attempt + 1}/{max_retries}): {e}")
                    time.sleep(wait)
...
def main():
    ...
    ap.add_argument("--out-prefix", type=str, default="corpus_embeddings_shard")
    ap.add_argument("--max-shard-mb", type=float, default=60,
                     help="Roll over to a new shard once the current one's uncompressed "
                          "size exceeds this (compressed output will be smaller, giving "
                          "safety margin under GitHub's 100MB limit)")
    args = ap.parse_args()
    ...
    shard_path = Path(f"{args.out_prefix}_{shard_index:03d}.jsonl.gz")
    f = gzip.open(shard_path, "wt", encoding="utf-8")
    shard_paths = [shard_path]
    print(f"Writing shard {shard_index}: {shard_path}")
    ...
    try:
        for i, pdf_path in enumerate(pdf_paths, 1):
            ...
            for c in chunks:
                ...
                f.write(line)
                shard_bytes_written += len(line.encode("utf-8"))

            print(f"  {len(chunks)} chunks")

            # Roll over to a new shard between books (never mid-book) once
            # the current shard's uncompressed size passes the threshold
            if shard_bytes_written >= max_shard_bytes:
                f.close()
                shard_index += 1
                shard_bytes_written = 0
                shard_path = Path(f"{args.out_prefix}_{shard_index:03d}.jsonl.gz")
    ...
    print(f"Wrote {len(shard_paths)} shard files: {[str(p) for p in shard_paths]}")
    print(f"{total_flagged} chunks flagged for possible character-loss review.")
    print(f"{total_embed_failed} chunks have a null embedding.")
    ...
    for p in shard_paths:
        ...
        if size_mb > 95:
            raise EmbeddingFailureError(
                f"{p} is {size_mb:.1f} MB, too close to GitHub's 100MB limit - "
                "lower --max-shard-mb and re-run."
            )

    if total_chunks > 0 and (total_embed_failed / total_chunks) > 0.02:
        raise EmbeddingFailureError(
            f"{total_embed_failed}/{total_chunks} chunks ({100 * total_embed_failed / total_chunks:.1f}%) "
            "failed to embed - this is too high to silently continue. "
            "Check the API key, billing status, and rate limits before re-running."
        )
